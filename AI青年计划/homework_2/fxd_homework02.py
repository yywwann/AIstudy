...
def normalization(X):
    Xmin = np.min(X,axis=0)#请补全                                # 请补全 通过使用 np.min 函数，计算原始数据沿着 axis=0 方向的最小值，即：求每一列的最小值，并赋值给 Xmin
    Xmax = np.max(X,axis=0)#请补全                                 # 请补全 通过使用 np.max 函数，计算原始数据沿着 axis=0 方向的最大值，即：求每一列的最大值，并赋值给 Xmax
    Xmu  = np.mean(X,axis=0)#请补全                               # 请补全 通过使用 np.mean 函数，计算原始数据均值，并赋值给 Xmu
...
def plotDecisionBoundary(trainX, trainY, w, iter_num = 0):
    ...
    fig_name = 'fxd_Training ' + str(iter_num) + ' times.png'
...
if __name__ == "__main__":
    ...
    plt_data.savefig('fxd_visualization_org.png')     # 通过调用 plt.savefig 函数，保存图像，并且图像的文件名为：'visualization_org.jpg'，其中，图片的格式为 'jpg'
    ...
    # optimizing by BSD
    iter_num=50000                                  # 进行初始化迭代的次数 iter_num，赋值 200 给 iter_num
