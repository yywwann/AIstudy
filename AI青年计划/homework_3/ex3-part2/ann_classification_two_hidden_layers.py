...
def calculate_loss(model, X, y):
    ...
    a2 = (np.row_stack((np.transpose(np.ones((a2.shape[1], 1))), a2)))  # 为矩阵a2增加一列值为1的偏置
    z3 = np.dot(W2, a2)  # 参数矩阵W2与a2做矩阵乘法，得到z3矩阵
    a3 = sigmoid(z3)  # 对z3矩阵进行sigmoid激活函数处理得到激活后的矩阵a3，即第二层隐藏层数值
    a3 = (np.row_stack((np.transpose(np.ones((a3.shape[1], 1))), a3)))  # 为矩阵a3增加一列值为1的偏置
...
def predict(model, X):
    ...
    a2 = (np.row_stack((np.transpose(np.ones((a2.shape[1], 1))), a2)))  # 为第一层隐藏层的矩阵a2增加一列值为1的偏置
    z3 = np.dot(W2, a2)  # 参数W2与a2做矩阵乘法
    a3 = sigmoid(z3)  # 对矩阵z3做sigmoid激活
    a3 = (np.row_stack((np.transpose(np.ones((a3.shape[1], 1))), a3)))  # 为第二层隐藏层的矩阵a3增加一列值为1的偏置
...
def build_model(X, y, hidden1_dim,hidden2_dim, iterNum=2000, print_loss=False):
    ...
    for t in range(0, iterNum):  # 从0循环至iterNum
        ...
        print(z2.shape)
        a2 = sigmoid(z2)  # 对矩阵z2进行做sigmoid激活
        a2 = (np.row_stack((np.transpose(np.ones((m, 1))), a2)))  # 为第一层隐藏层的矩阵a2增加一列值为1的偏置
        print(a2.shape)
        z3 = np.dot(W2, a2)  # 参数W2与a2做矩阵乘法
        print("z3 = ", z3.shape)
        a3 = sigmoid(z3)  # 对矩阵z3做sigmoid激活
        a3 = (np.row_stack((np.transpose(np.ones((m, 1))), a3)))  # 为第二层隐藏层的矩阵a3增加一列值为1的偏置
        z4 = np.dot(W3, a3)  # 参数W3与a3做矩阵乘法
        a4 = sigmoid(z4)  # 对矩阵z4做sigmoid激活
        print("a4 = ", a4.shape)
        ...
        print(delta4.shape)
        print(np.transpose(W3[:,1:]))
        delta3 = np.multiply(np.dot(np.transpose(W3[:,1:]), delta4), sigmoidGradient(z3))# 计算delta3，参数矩阵W3转置后与delta4做矩阵乘法，然后与sigmoidGradient(z3)对应位相乘
        delta2 = np.multiply(np.dot(np.transpose(W2[:,1:]), delta3), sigmoidGradient(z2))  # 计算delta2，参数矩阵W2转置后与delta3做矩阵乘法，然后与sigmoidGradient(z2)对应位相乘
