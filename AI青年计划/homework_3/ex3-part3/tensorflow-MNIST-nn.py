...
A1 = tf.layers.dense(inputs=x, units=96, activation=tf.nn.sigmoid)  #{0}为待补充, 添加全连接层，神经元个数为16个，激活函数为sigmoid、tanh或relu
A2 = tf.layers.dense(inputs=A1,units=96, activation=tf.nn.sigmoid)  #{0}为待补充，添加全连接层，神经元个数为16个，激活函数为sigmoid、tanh或relu
...
train_step = tf.train.GradientDescentOptimizer(0.05).minimize(cross_entropy) #设置随机梯度下降的学习率为0.01，最小化目标函数为cross_entropy
