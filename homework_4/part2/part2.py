...
def train(X, y,iterNum=2000):#定义训练函数,默认训练2000次
    ...
    for j in range(0, iterNum):
        for i in range(0, num):
            ...
            delta5 = np.dot(W5.T, delta6) * sigmoidGradient(z5)#delta6为第6层的“误差”
            delta4 = np.dot(W4.T, delta5) * sigmoidGradient(z4)#delta6为第6层的“误差”
            delta3 = np.dot(W3.T, delta4) * sigmoidGradient(z3)#delta6为第6层的“误差”
            delta2 = np.dot(W2.T, delta3) * sigmoidGradient(z2)#delta6为第6层的“误差”
